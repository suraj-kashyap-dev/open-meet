...
import type { UserInviteDto } from '@open-meet/types';
...
export function PendingUserInvites() {
  const t = useTranslations('users');
  const { data, isLoading, error } = useUserInvites();
  const resend = useResendUserInvite();
  const revoke = useRevokeUserInvite();
  const [busyId, setBusyId] = useState<string | null>(null);

  const invites = data?.items ?? [];

  const onResend = async (invite: UserInviteDto) => {
    setBusyId(invite.id);
    try {
  ...
  const onRevoke = async (invite: UserInviteDto) => {
    setBusyId(invite.id);
    try {
        ...
        return (
          <li key={invite.id} className="flex items-center gap-3 py-3">
            ...
            <p className="hidden text-[10px] text-muted-foreground sm:block">
              {t('pending.expires', { when: formatExpiry(invite.expiresAt) })}
            </p>
